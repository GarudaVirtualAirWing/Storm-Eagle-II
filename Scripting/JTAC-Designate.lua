...
-- RedRecceSetGroup = SET_GROUP:New():FilterCoalitions("red"):FilterPrefixes{ "JTAC", "_Tigr_" }:FilterStart() -- set only if PvP, PvE no needed!
...
BLUE_CAS_Set = SET_GROUP:New():FilterPrefixes( {"BATUMI ", "Kobuleti "} ):FilterStart()
--RED_CAS_Set = SET_GROUP:New():FilterPrefixes( {" Red AF", " Red Cargo"} ):FilterStart() -- set only if PvP, PvE no needed!
...
--RedRecceDetection = DETECTION_AREAS:New(RedRecceSetGroup, 10000)-- set only if PvP, PvE no needed!
--  :SetAcceptRange(10000)
--  :FilterCategories( { Unit.Category.GROUND_UNIT, Unit.Category.STRUCTURE } )
--  :SetAlphaAngleProbability( 0.9 )
...
BlueReconDesignation = DESIGNATE:New( BlueCommandCenter, BlueRecceDetection, BLUE_CAS_Set)
  ...
  :Detect()
--  :__Detect(-1)
-- set only if using for PvP, PvE no needed!
